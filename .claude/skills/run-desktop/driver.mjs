...
const COMMANDS = {
  async launch() {
    ...
    console.log('launched.', wins.length, 'windows:');
  ...
  async 'click-text'(text) {
    ...
    const r = await page.evaluate(t => {
      const els = [...document.querySelectorAll('button, a, [role="button"], li, td, th, div[class*="btn"], span[class*="btn"]')];
      const el = els.find(e => e.textContent?.trim() === t)
              ?? els.find(e => e.textContent?.includes(t));
      if (!el) return 'NOT_FOUND';
      el.click(); return 'OK: ' + el.tagName + ' "' + el.textContent?.trim().substring(0, 50) + '"';
    }, text);
    console.log('click-text', JSON.stringify(text), '→', r);
  },

  async type(text)  { if (page) await page.keyboard.type(text, { delay: 30 }); console.log('typed'); },
  ...
  async fill(args) {
    ...
    const r = await page.evaluate(([s, v]) => {
      ...
      if (nativeInputValueSetter) { nativeInputValueSetter.call(el, v); el.dispatchEvent(new Event('input', { bubbles: true })); }
      else { el.value = v; el.dispatchEvent(new Event('change', { bubbles: true })); }
      return 'OK';
    }, [sel, val]);
    console.log('fill', sel, '=', JSON.stringify(val), '→', r);
  },
...
console.log('Nexora POS driver — "help" for commands, "launch" to start');
rl.prompt();
