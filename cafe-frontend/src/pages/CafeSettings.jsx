...
export default function CafeSettings({ setPage }) {
  ...
  const printQR = (tableNum) => {
    const url  = `${window.location.origin}/play/${storeSlug}?table=${tableNum}`;
          ...
          {!storeSlug ? (
            ...
            <>
              <p style={{ color: "#78716c", fontSize: 14, marginBottom: 24 }}>
                Each QR code links to: <code style={{ background: "#f5ece0", padding: "2px 6px", borderRadius: 4, fontSize: 12 }}>play/{storeSlug}?table=N</code>
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 16 }}>
                {tables.map(t => {
                  const url = `${window.location.origin}/play/${storeSlug}?table=${t.number}`;
                  return (
